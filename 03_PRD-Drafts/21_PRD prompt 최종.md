...
6. **Target Market & Competitors (미국 시장 검증)**
   - Wombet2는 기본적으로 미국 시장을 대상으로 합니다. 따라서 경쟁사 분석과 차별화 벤치마크는 **반드시 미국 시장 경쟁사(예: Amazon, REI, Dick's Sporting Goods, RunSignup 등)**의 사례를 사용하세요.
   - ⚠️ 국내 시장에만 한정된 경쟁사(예: 무신사, 쿠팡 등)는 논의에서 철저히 배제하세요.

7. **문서 간 수치 데이터 정합성 (데이터 상충 시 원칙)**
   - 제공된 여러 문서 간 동일 지표에 대해 서로 다른 숫자가 존재하는 경우(예: VPS "8.2배" vs Deep Research "7.8배" 등), **본문은 항상 더 보수적인 수치를 채택**하여 기획의 건전성을 유지하세요.
   - 채택되지 않은 원문 수치는 문장 끝에 **각주(Footnote) 형태로 병기**하여 데이터 투명성을 확보하세요. (예: `탐색 비용 7.8배 증가[^1]` ... `[^1] 보수적 관점에 따라 Deep Research 데이터를 채택함. 원본 VPS 문서는 8.2배로 기재됨`)
