...
### 3.1 External Systems & Component Diagram
```mermaid
graph TD
    subgraph WOMBET2["WOMBET2 System Core"]
        WebApp["B2C Web App (Frontend)"]
        API["Core API Gateway"]
        TagEngine["Relativity Tagging Engine"]
        FitEngine["Match DNA Engine"]
        DB[(Product & Penalty DB)]
    end
    
    subgraph External["External Systems"]
        Amazon["Amazon Associates API"]
        B2BERP["B2B Partner ERP/Shopify"]
        Analytics["Google Analytics / CRM"]
    end
    
    WebApp -->|Requests Data| API
    WebApp -->|Redirects| Amazon
    API --> TagEngine
    API --> FitEngine
    API --> DB
    B2BERP -->|Fit API Calls| API
    WebApp -.->|Logging| Analytics
```
...
### 3.2 Client Applications & UseCase Diagram
```mermaid
flowchart LR
    %% Actors
    C1("C1 김러닝 (코어)"):::actor
    ...
    %% Use Cases
    subgraph WOMBET2 System
        UC1(["네거티브 필터 열람 (F1)"])
        UC2(["종목 기반 스펙 역산 (F3)"])
        UC3(["발 사진 업로드 및 진단 (F2)"])
        UC4(["안전 진단 오버레이 확인 (F4)"])
        UC5(["제휴 구매 링크 클릭"])
        UC6(["B2B 고객 핏 리스크 검증"])
        UC7(["단점 DB 관리 및 수동 진단"])
    end
...
### 3.4 Interaction Sequences (핵심 기능 흐름)

**[시퀀스 다이어그램 1: 시스템 전체 핵심 플로우]**
```mermaid
sequenceDiagram
    autonumber
...
**[시퀀스 다이어그램 2: 상대성 태깅 데이터 처리 흐름 (F3)]**
```mermaid
sequenceDiagram
    autonumber
    actor A1 as A1 조역도
    participant WebApp as WOMBET2 Web
    participant TagAPI as Relativity Tagging API
    participant DB as Product DB
    
    A1->>WebApp: 제품 카테고리 필터링 진입
    WebApp->>DB: 기본(범용) 평점 리스트 요청
    DB-->>WebApp: 범용 스펙 데이터 반환
    A1->>WebApp: 종목 드롭다운 선택 (역도)
    WebApp->>TagAPI: POST /api/v1/products/{id}/tags (sport_type=역도)
    TagAPI->>DB: 해당 종목 변환 스코어/로직 조회
    DB-->>TagAPI: 이종 스펙 필터링 및 역산 데이터 반환
    TagAPI-->>WebApp: 역산된 랭킹 및 오염 경고 라벨 응답
    WebApp-->>A1: 종목 맞춤형 리스트 및 장단점 표시
    
    opt 데이터 오류 인지 시
        A1->>WebApp: '부정확' 피드백 접수
        WebApp->>TagAPI: 피드백 신고 처리
        TagAPI-->>WebApp: 48시간 내 검증 트리거 알림
    end
```

### 3.5 미국 시장 경쟁 대안 벤치마크
...
|---|---|---|---|---|
| REQ-FUNC-001 | Story 1 / F1 | 네거티브 필터 최상단 노출 | **Given:** 제품 상세 진입<br>**When:** 로드 완료 시<br>**Then:** 붉은색 경고 박스 내 단점 3가지 노출<br>**Threshold:** LCP ≤ 1.5초, 뷰포트 상위 30% 이내 | Must / 1Sprint 구현 가능 |
...
| REQ-FUNC-004 | Story 2 / F3 | 목적별 스펙 역산 변환 | **Given:** 종목(역도) 선택<br>**When:** 리스트 갱신 시<br>**Then:** 목적별 역산 점수 반영 순위 표시<br>**Threshold:** 변환 정확도 ≥ 95%, p95 응답 ≤ 1초 | Must / 1Sprint 구현 가능 |
...
| REQ-FUNC-008 | Story 3 / F2 | 발 사진 기반 Match DNA 핏 진단 | **Given:** 사진 업로드<br>**When:** 핏 진단 요청 시<br>**Then:** 부위별 압박 위험도 히트맵 오버레이 제공<br>**Threshold:** 응답 ≤ 2시간 (MVP 수동), 자동화 후 ≤ 30초 | Must / 수동 진단 의존 |
...
| REQ-FUNC-010 | Story 3 / F4 | 고위험 판정 결제 직전 오버레이 | **Given:** 히트맵 상 '고위험' 판정 제품 구매 시도<br>**When:** 제휴 링크 클릭/결제 진입 전<br>**Then:** 시각적 경고 오버레이 필수 표시<br>**Threshold:** 경고 노출률 100% | Should / F2 데이터 의존 |
...
| REQ-NF-009 | Security | 발 사진 개인정보 보호 | CCPA/GDPR 준수, 진단 완료 후 원본 삭제 요청 시 즉시 파기 |
...
| REQ-NF-012 | Monitoring | CVR 저하 경보 | 제휴 CVR이 **< 3%** 연속 하락 시 즉각적인 Slack 경고 발송 |
| REQ-NF-013 | Monitoring | 기타 운영 알림 | 에러율 **> 0.5%** 시 PagerDuty 호출 / SLA 초과 시 알림 / 신규 모델 누락 시 알림 |
| REQ-NF-014 | Target KPI | 탐색 시간 및 CVR 목표 | 탐색 시간 **≤ 5분**으로 96% 절감, 제휴 구매 전환율(CVR) 보수적 **≥ 5.0%**, MAU 1년차 **50,000** |
...
|---|---|---|---|
| `/api/v1/products/{id}/penalties` | REST | Internal | 제품 상세 페이지 호출 시 F1 네거티브 필터 단점 데이터 조회 |
| `/api/v1/products/{id}/tags` | REST | Internal | 사용자가 종목 선택 시(F3) 역산된 상대성 태깅 스코어 리스트업 |
| `/api/v1/fit/diagnosis` | REST | B2B/Ext | 사용자 발 사진과 제품 ID를 기반으로 핏 리스크 및 JSON 히트맵 리턴 |
...
```mermaid
erDiagram
    PRODUCT ||--o{ NEGATIVE_FILTER : "has (1:N)"
    PRODUCT ||--o{ RELATIVITY_TAG : "has (1:N)"
    PRODUCT ||--o{ MATCH_DNA : "has (1:1/1:N)"
    PRODUCT ||--o{ FIT_DIAGNOSIS : "diagnosed for"
    USER_PROFILE ||--o{ FIT_DIAGNOSIS : "requests (1:N)"
...
classDiagram
    ...
    class RelativityTaggingEngine {
        +convertScore(baseScore: Float, sport: String): Float
        +filterContamination(specs: List): List
    }
    class MatchDNAAnalyzer {
        -SLA_LIMIT_HOURS: Float = 2.0
        +generateHeatmap(userFoot: FootData, productDna: DNA): JSON
        +assessFitRisk(heatmap: JSON): String
    }
    class AffiliateManager {
        +generateSecureLink(productId: String): String
        +trackConversion(clickId: String)
    }

    ProductController --> RelativityTaggingEngine : uses
    ProductController --> MatchDNAAnalyzer : triggers
    MatchDNAAnalyzer --> AffiliateManager : routes to checkout
```
...
**[시퀀스 다이어그램 3: Match DNA B2C 핏 진단 흐름]**
```mermaid
sequenceDiagram
    autonumber
...
**[시퀀스 다이어그램 4: B2B 파트너 API 연동 흐름 (Phase 2)]**
```mermaid
sequenceDiagram
    autonumber
    participant Customer as 제휴몰 고객
    participant B2BERP as B2B 파트너 (Shopify/ERP)
    participant FitAPI as Match DNA Fit API
    participant DB as WOMBET2 DB

    Customer->>B2BERP: 특정 신발 페이지 진입 및 핏 추천 요청
    B2BERP->>Customer: 발 사진/사이즈 업로드 프롬프트
    Customer-->>B2BERP: 사용자 발 데이터 제출
    B2BERP->>FitAPI: POST /api/v1/fit/diagnosis (user_data, product_id)
    FitAPI->>DB: 데이터 분석 및 기존 핏 DNA 대조
    DB-->>FitAPI: Risk Verdict 및 Heatmap 추출
    FitAPI-->>B2BERP: fit_risk_level (예: High), heatmap_overlay 응답
    
    alt fit_risk_level == High
        B2BERP-->>Customer: 해당 사이즈 반품 확률 경고 및 대체품 추천
    else fit_risk_level == Low
        B2BERP-->>Customer: 최적의 핏으로 안내 및 구매 유도
    end
```
